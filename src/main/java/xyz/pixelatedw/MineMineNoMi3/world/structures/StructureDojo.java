...
public class StructureDojo extends Structure {
  public static boolean build(Schematic sch, World world, int posX, int posY, int posZ, BiomeGenBase biome) {
    ...
    boolean flagBiome = (biome != BiomeGenBase.beach && biome != BiomeGenBase.plains && biome != BiomeGenBase.desert && biome != BiomeGenBase.savanna && biome != BiomeGenBase.icePlains && biome != BiomeGenBase.swampland && biome != BiomeGenBase.taiga && !biome.biomeName.equalsIgnoreCase((BiomeGenBase.plains.createMutation()).biomeName));
  ...
  private static void populate(int posX, int posY, int posZ, World world) {
    TileEntityCustomSpawner spawnDojoMaster = (new TileEntityCustomSpawner()).setSpawnerMob("mineminenomi.Dojo Sensei").setSpawnerLimit(1);
    world.setBlock(posX + 5, posY + 2, posZ + 11, (Block)ListMisc.CustomSpawner);
    world.setTileEntity(posX + 5, posY + 2, posZ + 11, (TileEntity)spawnDojoMaster);
  }
}
