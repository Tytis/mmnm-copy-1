...
public class KujaBow extends Item {
  ...
  public void onPlayerStoppedUsing(ItemStack itemStack, World world, EntityPlayer player, int time) {
    int j = getMaxItemUseDuration(itemStack) - time;
    ...
    if (flag || player.inventory.hasItem(ListMisc.KujaArrow)) {
      ...
      if (f > 1.0F)
        f = 1.0F; 
      if (f > 0.85D) {
        ExtraProjectiles.KujaArrow kujaArrow = new ExtraProjectiles.KujaArrow(world, (EntityLivingBase)player, ListExtraAttributes.KUJA_ARROW);
        ...
        if (!world.isRemote) {
          world.spawnEntityInWorld((Entity)kujaArrow);
  ...
  public void registerIcons(IIconRegister par1IconRegister) {
    this.itemIcon = par1IconRegister.registerIcon("mineminenomi:" + this.variation + "kujabow");
    this.iconArray = new IIcon[this.pullArray.length];
    for (int i = 0; i < this.iconArray.length; i++)
      this.iconArray[i] = par1IconRegister.registerIcon("mineminenomi:" + this.pullArray[i]); 
  }
