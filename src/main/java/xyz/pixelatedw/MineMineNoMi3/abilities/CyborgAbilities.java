  ...
  public static class CoupDeVent extends Ability {
    ...
    public void endCharging(EntityPlayer player) {
      ...
      for (int i = 0; i < 100; i++) {
        ...
        this.projectile = (AbilityProjectile)new CyborgProjectiles.CoupDeVent(player.worldObj, (EntityLivingBase)player, this.attr);
        this.projectile.setLocationAndAngles(player.posX + offsetX, player.posY + 2.0D + offsetY, player.posZ + offsetZ, player.cameraPitch, player.cameraYaw);
    ...
    public void use(EntityPlayer player) {
      ...
      if (!player.worldObj.isRemote)
        if (!this.isOnCooldown && props.getCola() >= 10) {
          this.projectile = (AbilityProjectile)new CyborgProjectiles.StrongRight(player.worldObj, (EntityLivingBase)player, this.attr);
          player.worldObj.spawnEntityInWorld((Entity)this.projectile);
          props.alterCola(-10);
          WyNetworkHelper.sendTo((IMessage)new PacketSync(props), (EntityPlayerMP)player);
          super.use(player);
    ...
    public void use(EntityPlayer player) {
      ...
      if (!player.worldObj.isRemote)
        if (!this.isOnCooldown && props.getCola() >= 15) {
          this.projectile = (AbilityProjectile)new CyborgProjectiles.RadicalBeam(player.worldObj, (EntityLivingBase)player, this.attr);
          player.worldObj.spawnEntityInWorld((Entity)this.projectile);
          props.alterCola(-15);
          WyNetworkHelper.sendTo((IMessage)new PacketSync(props), (EntityPlayerMP)player);
          super.use(player);
    ...
    public void use(EntityPlayer player) {
      ...
      if (!this.isOnCooldown && props.getCola() >= 5) {
        for (int i = 0; i < 100; i++) {
          ...
          double offsetY = ((new Random()).nextInt(50) + 1.0D - 25.0D) / 20.0D;
          double offsetZ = ((new Random()).nextInt(50) + 1.0D - 25.0D) / 20.0D;
          this.projectile = (AbilityProjectile)new CyborgProjectiles.FreshFire(player.worldObj, (EntityLivingBase)player, this.attr);
          this.projectile.setLocationAndAngles(player.posX + offsetX, player.posY + 2.0D + offsetY, player.posZ + offsetZ, player.cameraPitch, player.cameraYaw);
          player.worldObj.spawnEntityInWorld((Entity)this.projectile);
        } 
        props.alterCola(-5);
  ...
  public static class ColaOverdrive extends Ability {
    ...
    public void use(EntityPlayer player) {
      ...
      if (!this.isOnCooldown && props.getCola() > 0) {
        ...
        props.setCola(0);
        player.setHealth((float)(player.getHealth() + r / 100.0D * player.getEntityAttribute(SharedMonsterAttributes.maxHealth).getBaseValue()));
        WyNetworkHelper.sendTo((IMessage)new PacketSync(props), (EntityPlayerMP)player);
        super.use(player);
