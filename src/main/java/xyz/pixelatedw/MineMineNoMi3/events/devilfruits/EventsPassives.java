...
public class EventsPassives {
    ...
    if (event.entityLiving instanceof EntityPlayer) {
      ...
      if (props.getUsedFruit().equals("supasupa"))
        if (atomicSpurt != null && atomicSpurt.isPassiveActive())
          if (player.onGround) {
            if (Math.abs(player.motionX) < 0.5D || Math.abs(player.motionZ) < 0.5D) {
              player.motionX *= 1.6D;
              player.motionZ *= 1.6D;
            } 
            if (Math.abs(player.motionX) > 0.15D || Math.abs(player.motionZ) > 0.15D)
      ...
      if (props.getUsedFruit().equals("yomiyomi") && props.getZoanPoint().equalsIgnoreCase("yomi")) {
        ...
        if (WyHelper.getEntitiesNear((Entity)player, 100.0D, new Class[] { EntityPlayer.class }).size() > 0 && player.ticksExisted % 500 == 0)
      ...
      if (DevilFruitsHelper.isAffectedByWater((EntityLivingBase)player))
      ...
      if (!player.worldObj.isRemote)
        if (props.hasExtraEffects("haoHaki") && player.isPotionActive(Potion.blindness)) {
    ...
    if (event.source.getSourceOfDamage() instanceof EntityLivingBase && event.entityLiving instanceof EntityPlayer) {
      ...
      if (props.getUsedFruit().equalsIgnoreCase("yomiyomi") && props.getZoanPoint().equalsIgnoreCase("yomi")) {
        ...
        if (soulParade != null && soulParade.isPassiveActive()) {
          ...
          AbilityExplosion explosion = WyHelper.newExplosion((Entity)attacked, attacker.posX, attacker.posY, attacker.posZ, 2.0D);
    ...
    if (event.source.getSourceOfDamage() instanceof EntityPlayer) {
      ...
      if (props.getUsedFruit().equalsIgnoreCase("kilokilo"))
        if (attacker.isPotionActive(Potion.damageBoost)) {
          WyNetworkHelper.sendToAllAround((IMessage)new PacketParticles("kilo", attacked.posX, attacked.posY, attacked.posZ), attacker.dimension, attacker.posX, attacker.posY, attacker.posZ, 128.0D);
          attacker.removePotionEffect(Potion.damageBoost.id);
        }  
      if (props.getUsedFruit().equalsIgnoreCase("kagekage")) {
        EntityDoppelman doppelman = (EntityDoppelman) WyHelper.getEntitiesNear((Entity)attacker, 20.0D, new Class[] { EntityDoppelman.class }).stream().findFirst().orElse(null);
        if (doppelman != null)
      ...
      if (hardeningBuso != null && hardeningBuso.isPassiveActive()) {
        ...
        event.ammount = (float)(event.ammount + power * props.getDamageMultiplier());
      } 
  ...
  public void onEntityShootProjectile(ArrowLooseEvent event) {
    if (event.entityPlayer != null) {
      ExtendedEntityData props = ExtendedEntityData.get((EntityLivingBase)event.entityPlayer);
  ...
  public void onDamage(LivingAttackEvent event) {
    if (event.entityLiving instanceof EntityPlayer) {
      ...
      if (props.isInAirWorld())
        event.setCanceled(true); 
      if (player.getHealth() - event.ammount <= 0.0F)
        if (props.getUsedFruit().equalsIgnoreCase("yomiyomi") && !props.getZoanPoint().equalsIgnoreCase("yomi")) {
          props.setUsedFruit("yomiyomi");
          props.setZoanPoint("yomi");
          AbilityExplosion explosion = WyHelper.newExplosion((Entity)player, player.posX, player.posY, player.posZ, 2.0D);
          explosion.setDamageOwner(false);
      ...
      if (event.source.getSourceOfDamage() instanceof EntityLivingBase)
        if (event.entityLiving instanceof EntityPlayer) {
          ...
          ExtendedEntityData propz = ExtendedEntityData.get((EntityLivingBase)reciever);
          HakiAbilities.KenbunshokuHakiFutureSight futureSight = (HakiAbilities.KenbunshokuHakiFutureSight)abilityProps.getAbilityFromName(ListAttributes.KENBUNSHOKU_HAKI_FUTURE_SIGHT.getAttributeName());
