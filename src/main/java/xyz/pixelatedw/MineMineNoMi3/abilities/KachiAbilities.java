...
public class KachiAbilities {
  ...
  public static class HotBoilingSpecial extends Ability {
    ...
    public void duringPassive(EntityPlayer player, int timer) {
      player.addPotionEffect(new PotionEffect(11, 8, 0));
    }
  ...
  public static class Evaporate extends Ability {
    ...
    public void use(EntityPlayer player) {
      if (!this.isOnCooldown) {
        List<int[]> coords = WyHelper.createFilledSphere(player.getEntityWorld(), (int)player.posX, (int)player.posY, (int)player.posZ, 6, Blocks.air, new String[] { "liquids" });
        for (int count = 0; count < coords.size(); count++) {
          ...
          if (player.getEntityWorld().getBlock(ints[0], ints[1], ints[2]).equals(Blocks.air))
            WyNetworkHelper.sendToAllAround((IMessage)new PacketParticles("evaporate", ints[0], ints[1], ints[2]), player.dimension, ints[0], ints[1], ints[2], 128.0D); 
        } 
      } 
      super.use(player);
    }
