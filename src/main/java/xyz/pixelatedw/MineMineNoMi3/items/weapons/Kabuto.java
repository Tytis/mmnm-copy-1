...
public class Kabuto extends Item {
  ...
  public void onPlayerStoppedUsing(ItemStack itemStack, World world, EntityPlayer player, int time) {
    ...
    if (flag || player.inventory.hasItem(ListMisc.PopGreen)) {
      ...
      if (f > 1.0F)
        f = 1.0F; 
      if (f > 0.85D) {
        ExtraProjectiles.PopGreen popGreen = new ExtraProjectiles.PopGreen(world, (EntityLivingBase)player, ListExtraAttributes.POP_GREEN);
        ...
        if (!world.isRemote) {
          world.spawnEntityInWorld((Entity)popGreen);
  ...
  public void registerIcons(IIconRegister par1IconRegister) {
    this.itemIcon = par1IconRegister.registerIcon("mineminenomi:" + this.kabutoType);
