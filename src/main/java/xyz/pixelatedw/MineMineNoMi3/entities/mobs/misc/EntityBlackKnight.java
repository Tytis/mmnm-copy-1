...
public class EntityBlackKnight extends EntityMob {
  ...
  public void onEntityUpdate() {
    ...
    if (!this.worldObj.isRemote && this.owner != null) {
      ...
      List<EntityLivingBase> attackList = this.isAggressive ? WyHelper.getEntitiesNear((Entity)this, 10.0D, new Class[] { EntityPlayer.class, MarineData.class, PirateData.class }) : (!this.forcedTargets.isEmpty() ? this.forcedTargets : new ArrayList<>());
      if (!ownerProps.getUsedFruit().equalsIgnoreCase("itoito"))
        setDead(); 
      if (!attackList.isEmpty()) {
        if (attackList.contains(this.owner))
          attackList.remove(this.owner); 
        if (ownerProps.isMarine())
          attackList = (List<EntityLivingBase>)attackList.stream().filter(x -> !(x instanceof MarineData)).collect(Collectors.toList()); 
