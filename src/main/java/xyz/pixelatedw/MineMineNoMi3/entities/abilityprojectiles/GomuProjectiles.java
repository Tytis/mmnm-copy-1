  ...
  public static class GomuGomuNoRocket extends AbilityProjectile {
    ...
    public void tasksImapct(MovingObjectPosition hit) {
      ...
      if (hit.entityHit == null) {
        ...
        double mY = 0.0D;
        double mZ = 0.0D;
        double powerX = Math.abs(hit.blockX - player.posX) / 5.0D;
        double powerY = (hit.blockY - player.posY) / 4.0D;
        double powerZ = Math.abs(hit.blockZ - player.posZ) / 5.0D;
        mY += powerY;
        ...
        GomuProjectiles.motion("=", mX, mY, mZ, player);
      } 
    }
  }
  ...
  public static class GomuGomuNoLeoBazooka extends AbilityProjectile {
    ...
    public void tasksImapct(MovingObjectPosition hit) {
      if (hit.entityHit != null && hit.entityHit instanceof EntityLivingBase) {
        ...
        int maxPower = 17;
        newPosY += 2.0D;
        WyHelper.Direction dir = WyHelper.get4Directions((Entity)getThrower());
        if (dir == WyHelper.Direction.SOUTH) {
