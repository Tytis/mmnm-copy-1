...
public class PacketNewAABB implements IMessage {
  ...
  public static class ClientHandler implements IMessageHandler<PacketNewAABB, IMessage> {
    ...
    public IMessage onMessage(PacketNewAABB message, MessageContext ctx) {
      ...
      ((EntityPlayer)entityClientPlayerMP).width = message.sizeX;
      ((EntityPlayer)entityClientPlayerMP).height = message.sizeY;
      double minX = ((EntityPlayer)entityClientPlayerMP).boundingBox.minX;
      double minY = ((EntityPlayer)entityClientPlayerMP).boundingBox.minY;
      double minZ = ((EntityPlayer)entityClientPlayerMP).boundingBox.minZ;
      double maxX = minX + message.sizeX;
      double maxY = minY + message.sizeY;
      double maxZ = minZ + message.sizeZ;
      ((EntityPlayer)entityClientPlayerMP).boundingBox.setBounds(minX, minY, minZ, maxX, maxY, maxZ);
      return null;
    }
  }
  ...
  public static class ServerHandler implements IMessageHandler<PacketNewAABB, IMessage> {
    public IMessage onMessage(PacketNewAABB message, MessageContext ctx) {
      ...
      double maxY = minY + message.sizeY;
      double maxZ = minZ + message.sizeZ;
      player.boundingBox.setBounds(minX, minY, minZ, maxX, maxY, maxZ);
