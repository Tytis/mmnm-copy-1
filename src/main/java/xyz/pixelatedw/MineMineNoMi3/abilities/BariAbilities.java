...
public class BariAbilities {
    ...
    public void passive(EntityPlayer player) {
      if (!this.isOnCooldown) {
        if (this.blockList.isEmpty())
          player.worldObj.spawnEntityInWorld((Entity)new BariProjectiles.BarrierbilityStairs(player.worldObj, (EntityLivingBase)player, this.attr)); 
  ...
  public static class BarrierCrash extends Ability {
    ...
    public void use(EntityPlayer player) {
      this.projectile = (AbilityProjectile)new BariProjectiles.BarrierCrash(player.worldObj, (EntityLivingBase)player, this.attr);
      super.use(player);
    }
  }
      ...
      if (!this.isOnCooldown) {
        if (this.blockList.isEmpty()) {
          ...
          if (player.isSneaking()) {
            this.blockList.addAll(WyHelper.createEmptySphere(world, (int)player.posX, (int)player.posY, (int)player.posZ, 5, ListMisc.Barrier, new String[] { "air", "foliage", "nogrief" }));
          } else if (mop != null && world.getBlock(mop.blockX, mop.blockY, mop.blockZ) != Blocks.air) {
            this.blockList.addAll(WyHelper.createEmptySphere(world, mop.blockX, mop.blockY, mop.blockZ, 5, ListMisc.Barrier, new String[] { "air", "foliage", "nogrief" }));
          } 
        } 
        super.passive(player);
      } 
      ...
      if (!this.isOnCooldown) {
        if (this.blockList.isEmpty()) {
          if (WyHelper.get4Directions((Entity)player) == WyHelper.Direction.NORTH)
            this.blockList = WyHelper.createFilledCube(player.worldObj, player.posX, player.posY, player.posZ - 3.0D, new int[] { 3, 4, 1 }, ListMisc.Barrier, new String[] { "air", "foliage", "nogrief" }); 
          ...
          if (WyHelper.get4Directions((Entity)player) == WyHelper.Direction.EAST)
            this.blockList = WyHelper.createFilledCube(player.worldObj, player.posX + 3.0D, player.posY, player.posZ, new int[] { 1, 4, 3 }, ListMisc.Barrier, new String[] { "air", "foliage", "nogrief" }); 
