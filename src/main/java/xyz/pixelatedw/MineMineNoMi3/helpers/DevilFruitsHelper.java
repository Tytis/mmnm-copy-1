...
public class DevilFruitsHelper {
  ...
  private static HashMap<String, List<Block>> createBlockRulesMap() {
    ...
    map.put("core", Arrays.asList(new Block[] { 
            Blocks.ice, Blocks.packed_ice, Blocks.stone, (Block)Blocks.grass, Blocks.dirt, Blocks.snow, Blocks.snow_layer, (Block)Blocks.sand, Blocks.sandstone, Blocks.sandstone_stairs, 
            Blocks.wooden_door, (Block)Blocks.wooden_slab, Blocks.log, Blocks.log2, Blocks.carpet, Blocks.cake, ListMisc.Poison, ListMisc.DemonPoison, Blocks.torch, Blocks.redstone_torch, 
            (Block)Blocks.redstone_wire, Blocks.cobblestone, Blocks.fence, Blocks.farmland, Blocks.fence_gate, Blocks.flower_pot, Blocks.clay, Blocks.gravel, ListMisc.SunaSand, ListMisc.WaxBlock }));
    map.put("air", Arrays.asList(new Block[] { Blocks.air }));
    map.put("foliage", Arrays.asList(new Block[] { 
            (Block)Blocks.leaves, (Block)Blocks.leaves2, Blocks.waterlily, (Block)Blocks.double_plant, (Block)Blocks.yellow_flower, (Block)Blocks.red_flower, Blocks.vine, (Block)Blocks.brown_mushroom, Blocks.brown_mushroom_block, (Block)Blocks.red_mushroom, 
            Blocks.red_mushroom_block, (Block)Blocks.tallgrass, Blocks.potatoes, Blocks.carrots, Blocks.cactus, (Block)Blocks.deadbush }));
    map.put("ores", Arrays.asList(new Block[] { 
            Blocks.coal_ore, Blocks.coal_block, Blocks.diamond_ore, Blocks.diamond_block, Blocks.iron_ore, Blocks.iron_block, Blocks.lapis_ore, Blocks.lapis_block, Blocks.redstone_ore, Blocks.redstone_block, 
            Blocks.gold_ore, Blocks.gold_block, ListMisc.KairosekiOre, ListMisc.KairosekiBlock }));
    map.put("liquids", Arrays.asList(new Block[] { Blocks.water, (Block)Blocks.flowing_water, Blocks.lava, (Block)Blocks.flowing_lava }));
    return map;
  }
  
  public static void haoAttackEntities(EntityPlayer player) {
    ExtendedEntityData props = ExtendedEntityData.get((EntityLivingBase)player);
    for (EntityLivingBase target : WyHelper.getEntitiesNear((Entity)player, 100.0D)) {
      double userHakiExp = (props.getHardeningHakiExp() * 3 + props.getImbuingHakiExp() * 3 + props.getObservationHakiExp() * 3);
      double targetHakiExp = 0.0D;
      ...
      if (targetHakiExp < userHakiExp / 1.5D) {
        int duration = (int)((userHakiExp / 1.5D - targetHakiExp) * 20.0D);
        if (duration > 2000)
          duration = 2000; 
        ...
        if (targetHakiExp < userHakiExp / 2.0D) {
  ...
  public static void sendShounenScream(EntityPlayer player, String ability, int part) {
    if (MainConfig.enableAnimeScreaming)
      WyNetworkHelper.sendToAllAround((IMessage)new PacketShounenScream(player.getCommandSenderName(), ability, part), player.dimension, player.posX, player.posY, player.posZ, 15.0D); 
  }
  ...
  public static boolean isAffectedByWater(EntityLivingBase entity) {
    ...
    if (entity.isInsideOfMaterial(Material.water) || (entity
      .isWet() && !entity.isRiding() && (level1Block == Blocks.water || level1Block == Blocks.flowing_water) && (level2Block == Blocks.water || level2Block == Blocks.flowing_water)))
      return true; 
    return false;
  }
  
  public static boolean isNearbyKairoseki(EntityPlayer player) {
    if (WyHelper.isBlockNearby((EntityLivingBase)player, 4, new Block[] { ListMisc.KairosekiBlock, ListMisc.KairosekiOre, ListMisc.KairosekiBars }) || ItemsHelper.hasKairosekiItem(player) || player.inventory.hasItem(Item.getItemFromBlock(ListMisc.KairosekiBlock)) || player.inventory.hasItem(Item.getItemFromBlock(ListMisc.KairosekiOre)) || player.inventory.hasItem(Item.getItemFromBlock(ListMisc.KairosekiBars)) || isAffectedByWater((EntityLivingBase)player))
      return true; 
    return false;
  }
  ...
  public static ItemStack getDevilFruitItem(String fullName) {
    ...
    if (fullName.equals("yamidummy"))
      fullName = "yamiyami"; 
    return new ItemStack(GameRegistry.findItem("mineminenomi", fullName.replace(model, "") + "nomi" + fullModel));
  }
