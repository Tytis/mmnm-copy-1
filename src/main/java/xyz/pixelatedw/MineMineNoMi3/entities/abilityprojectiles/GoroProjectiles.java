...
public class GoroProjectiles {
    ...
    public void onUpdate() {
      if (this.worldObj.isRemote)
        for (int i = 0; i < DevilFruitsHelper.getParticleSettingModifier(2); i++) {
          ...
          EntityParticleFX particle = (new EntityParticleFX(this.worldObj, particleToUse, this.posX + this.worldObj.rand.nextDouble(), this.posY + this.worldObj.rand.nextDouble() - 0.5D, this.posZ + this.worldObj.rand.nextDouble(), 0.0D, 0.0D, 0.0D)).setParticleAge(5).setParticleScale(4.0F);
    ...
    public void onUpdate() {
      if (this.worldObj.isRemote)
        for (int i = 0; i < DevilFruitsHelper.getParticleSettingModifier(35); i++) {
          ...
          EntityParticleFX particle = (new EntityParticleFX(this.worldObj, ID.PARTICLE_ICON_GORO2, this.posX + offsetX + this.worldObj.rand.nextDouble(), this.posY + offsetY + this.worldObj.rand.nextDouble(), this.posZ + offsetZ + this.worldObj.rand.nextDouble(), 0.0D, 0.0D, 0.0D)).setParticleAge(10).setParticleScale(10.0F);
          MainMod.proxy.spawnCustomParticles((Entity)this, particle);
        }  
      super.onUpdate();
    }
  ...
  public static class VoltVari5Million extends AbilityProjectile {
    ...
    public void onUpdate() {
      if (this.worldObj.isRemote)
        for (int i = 0; i < DevilFruitsHelper.getParticleSettingModifier(2); i++) {
          EntityParticleFX particle = (new EntityParticleFX(this.worldObj, ID.PARTICLE_ICON_GORO2, this.posX + this.worldObj.rand.nextDouble() - 0.5D, this.posY + this.worldObj.rand.nextDouble() - 0.5D, this.posZ + this.worldObj.rand.nextDouble() - 0.5D, 0.0D, 0.0D, 0.0D)).setParticleAge(10).setParticleScale(2.0F);
          MainMod.proxy.spawnCustomParticles((Entity)this, particle);
        }  
      super.onUpdate();
    }
