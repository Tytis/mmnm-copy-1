  ...
  public static class Moko extends AbilityProjectile {
    ...
    public void onUpdate() {
      if (this.worldObj.isRemote) {
        ...
        EntityParticleFX particle = (new EntityParticleFX(this.worldObj, ID.PARTICLE_ICON_GASU, this.posX + posXOffset, this.posY + posYOffset, this.posZ + posZOffset, 0.0D, 0.0D, 0.0D)).setParticleAge(20).setParticleScale(0.7F);
      ...
      if (hit.entityHit != null && hit.entityHit instanceof EntityLivingBase) {
        for (int x = -1; x < 1; x++) {
          for (int z = -1; z < 1; z++) {
            ...
            if (DevilFruitsHelper.placeBlockIfAllowed(this.worldObj, posX, posY, posZ, Blocks.air, new String[] { "all", "restricted", "ignore liquid" })) {
              hit.entityHit.motionX = 0.0D;
              hit.entityHit.motionZ = 0.0D;
              hit.entityHit.motionY -= 5.0D;
              ((EntityLivingBase)hit.entityHit).addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 100, 10));
            } 
          } 
        } 
      } else {
        DevilFruitsHelper.placeBlockIfAllowed(this.worldObj, hit.blockX, hit.blockY, hit.blockZ, Blocks.air, new String[] { "all", "restricted", "ignore liquid" });
      } 
    }
  }
