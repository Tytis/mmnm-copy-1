...
public class MainConfig {
  ...
  public static void init(File configFile) {
    ...
    config.load();
    enableKeepIEEPAfterDeath = config.get("general", "Keep stats after death", "auto", "Responsible for how player stats are handled after death; \nnone - nothing is kept; \nauto - faction/race/fight style stats are kept everything else is reseted; \nfull - everything is kept; \ncustom - uses the Data to Keep option to determine what is kept and what is reseted; auto by default").getString();
    haoshokuHakiUnlockLogic = config.get("general", "Haoshoku Haki Unlock Logic", "exp", "Responsible for how player unlock Haoshoku Haki; \nnone - Nobody can unlock it; \nexp - Based on the total haki experience earned, meaning mastering other haki forms will unlock it; \nrandom - Only a few chosen ones, randomlly selected, will unlock it; \nexp by default").getString();
    statsToKeep = config.get("general", "Data to Keep", new String[] { "Doriki", "Bounty", "Belly", "Race", "Faction", "Fighting Style", "Devil Fruit" }).getStringList();
    enableGriefing = config.get("general", "Allow Griefing in Worlds", true, "Allows abilities to break or replace blocks, this will make some abilities completly useless; true by default").getBoolean();
    enableAnimeScreaming = config.get("general", "Anime Screaming", false, "Will send a chat message to nearby players with the used ability's name; false by default").getBoolean();
    enableSpecialFlying = config.get("general", "Allow Special Flying", false, "Allows Gasu Gasu no Mi, Moku Moku no Mi and Suna Suna no Mi users to fly, this option does not affect flying Zoans which will be able to fly regardless; false by default").getBoolean();
    enableOneFruitPerWorld = config.get("general", "Allow Only One Fruit Per World", false, "Restricts the Devil Fruit spawns to only 1 of each type per world; false by default").getBoolean();
    enableYamiSpecialPower = config.get("general", "Allow Yami Users to eat another Devil Fruit", true, "Allows Yami Yami no Mi users to eat an additional fruit; true by default").getBoolean();
    enableDFtoDrop = config.get("general", "Allow Devil Fruits to drop from leaves", false).getBoolean();
    rateDFDrops = config.get("general", "Rate at which Devil Fruits drop from leaves", 1.0D).getDouble();
    enableLogiaInvulnerability = config.get("general", "Allow Logia Invulnerability", true, "Allows logia users to avoid physical attacks; true by default").getBoolean();
    enableExtraHearts = config.get("general", "Receive Extra Hearts", true, "Allows players to receive extra hearts based on their doriki; true by default").getBoolean();
    enableMobRewards = config.get("general", "Allow Mob Rewards", true, "Allows mobs to reward doriki, bounty or items; true by default").getBoolean();
    modifierDorikiReward = config.get("general", "Modifier for Doriki Reward", 1.0D, "Multiplier for the doriki reward when killing a mob").getDouble();
    enableMinimumDorikiPerKill = config.get("general", "Minimum Doriki per Kill", false, "Ensures a minimum of 1 doriki is gained after each kill").getBoolean();
    enableBossFights = config.get("general", "Allow Boss Fights", true, "Allow boss entires to be added in the History tab and players to fight them").getBoolean();
    dorikiKeepPercentage = config.get("general", "Percentage of doriki kept after death", 33, "Percentage of doriki kept after death; \n33% by default; \napplied for both 'auto' and 'custom' settings").getInt();
    bountyKeepPercentage = config.get("general", "Percentage of bounty kept after death", 0, "Percentage of bounty kept after death; \n0% by default; \napplied for both 'auto' and 'custom' settings").getInt();
    bellyKeepPercentage = config.get("general", "Percentage of belly kept after death", 0, "Percentage of belly kept after death; \n0% by default; \napplied for both 'auto' and 'custom' settings").getInt();
    enableShips = config.get("structures", "Allow Ships to Spawn", true).getBoolean();
    modifierShipsSpawn = config.get("structures", "Modifier for Spawning Ships", 5.0D).getDouble();
