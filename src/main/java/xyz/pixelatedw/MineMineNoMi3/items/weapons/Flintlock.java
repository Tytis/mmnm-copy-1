...
public class Flintlock extends Item {
  public ItemStack onItemRightClick(ItemStack itemStack, World world, EntityPlayer player) {
    if (player.isSneaking()) {
    ...
    } else if (itemStack.getTagCompound().getBoolean("canUse")) {
      if (itemStack.getTagCompound().getInteger("gunPowder") > 0) {
        if ((player.inventory.hasItem(ListMisc.Bullets) && itemStack.getTagCompound().getInteger("bulletType") == 0) || (player.inventory
          .hasItem(ListMisc.KairosekiBullets) && itemStack.getTagCompound().getInteger("bulletType") == 1)) {
          AbilityProjectile proj = null;
          int powder = itemStack.getTagCompound().getInteger("gunPowder");
          if (!world.isRemote) {
        	AbilityProjectile bullet = new ExtraProjectiles.NormalBullet(player.worldObj, (EntityLivingBase)player, ListExtraAttributes.NORMAL_BULLET);;
            if (itemStack.getTagCompound().getInteger("bulletType") == 0) {
            	bullet = new ExtraProjectiles.NormalBullet(player.worldObj, (EntityLivingBase)player, ListExtraAttributes.NORMAL_BULLET);
            } else if (itemStack.getTagCompound().getInteger("bulletType") == 1) {
            	bullet = new ExtraProjectiles.KairosekiBullet(player.worldObj, (EntityLivingBase)player, ListExtraAttributes.KAIROSEKI_BULLET);
            } 
      ...
      } else if (player.inventory.hasItem(Items.gunpowder)) {
