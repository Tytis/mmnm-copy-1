...
public class FishKarateAbilities {
  ...
  public static class Uchimizu extends Ability {
    ...
    public void use(EntityPlayer player) {
      this.projectile = (AbilityProjectile)new FishKarateProjectiles.Uchimizu(player.worldObj, (EntityLivingBase)player, this.attr);
      super.use(player);
    }
  }
  ...
  public static class Murasame extends Ability {
    ...
    public void use(EntityPlayer player) {
      this.projectile = (AbilityProjectile)new FishKarateProjectiles.Soshark(player.worldObj, (EntityLivingBase)player, this.attr);
      super.use(player);
    }
  }
  ...
  public static class SamehadaShotei extends Ability {
    ...
    public void duringPassive(EntityPlayer player, int passiveTimer) {
      WyNetworkHelper.sendToAllAround((IMessage)new PacketParticles("samehada", (EntityLivingBase)player), player.dimension, player.posX, player.posY, player.posZ, 128.0D);
      if (passiveTimer > 800) {
  ...
  public static class KarakusagawaraSeiken extends Ability {
    ...
    public void use(EntityPlayer player) {
      if (!this.isOnCooldown) {
        for (EntityLivingBase elb : WyHelper.getEntitiesNear((Entity)player, 10.0D))
          elb.attackEntityFrom(DamageSource.causePlayerDamage(player), 20.0F); 
        super.use(player);
      } 
    }
  }
