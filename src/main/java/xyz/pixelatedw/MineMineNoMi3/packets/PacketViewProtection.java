...
public class PacketViewProtection implements IMessage {
  ...
  public static class ClientHandler implements IMessageHandler<PacketViewProtection, IMessage> {
    ...
    public IMessage onMessage(PacketViewProtection message, MessageContext ctx) {
      ...
      WyHelper.createEmptyCube(((EntityPlayer)entityClientPlayerMP).worldObj, message.midPoint[0], message.midPoint[1], message.midPoint[2], new int[] { message.radius, message.radius, message.radius }, ListMisc.AbilityProtectionAreaBlock, new String[] { "air", "liquids" });
      WyHelper.createEmptySphere(((EntityPlayer)entityClientPlayerMP).worldObj, message.midPoint[0], message.midPoint[1], message.midPoint[2], 1, ListMisc.AbilityProtectionCenterBlock, new String[] { "air", "liquids" });
      return null;
    }
  }
}
