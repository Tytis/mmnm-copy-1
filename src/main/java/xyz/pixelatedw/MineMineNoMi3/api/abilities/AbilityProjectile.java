...
public class AbilityProjectile extends EntityThrowable {
  ...
  public AbilityProjectile(World world, EntityLivingBase player, AbilityAttribute attr) {
    ...
    this.user = player;
    if (getThrower() != null && getThrower() instanceof EntityPlayer && DevilFruitsHelper.checkForRestriction((EntityPlayer)getThrower()))
      setDead(); 
    if (this.attr != null) {
      setLocationAndAngles(this.user.posX, this.user.posY + this.user.getEyeHeight(), this.user.posZ, this.user.rotationYaw, this.user.rotationPitch);
      this.motionX = (-MathHelper.sin(this.rotationYaw / 180.0F * 3.1415927F) * MathHelper.cos(this.rotationPitch / 180.0F * 3.1415927F)) * 0.4D;
      this.motionZ = (MathHelper.cos(this.rotationYaw / 180.0F * 3.1415927F) * MathHelper.cos(this.rotationPitch / 180.0F * 3.1415927F)) * 0.4D;
      this.motionY = -MathHelper.sin((this.rotationPitch + func_70183_g()) / 180.0F * 3.1415927F) * 0.4D;
      setThrowableHeading(this.motionX, this.motionY, this.motionZ, attr.getProjectileSpeed(), 1.0F);
    } 
  }
  ...
  public void onUpdate() {
    ...
    if (this.attr != null) {
      ...
      Entity entity = null;
      List<Entity> list = this.worldObj.getEntitiesWithinAABBExcludingEntity((Entity)this, this.boundingBox.addCoord(this.motionX, this.motionY, this.motionZ).expand(sizeX, sizeY, sizeZ));
      double d0 = 0.0D;
      for (int i = 0; i < list.size(); i++) {
        Entity entity1 = list.get(i);
        if (entity1.canBeCollidedWith() && (entity1 != getThrower() || this.ticksExisted >= 5)) {
    ...
    if (!this.worldObj.isRemote)
      if (this.attr != null) {
        if (hit.entityHit != null && hit.entityHit instanceof EntityLivingBase) {
          ExtendedEntityData props = ExtendedEntityData.get(getThrower());
          ExtendedEntityData propz = ExtendedEntityData.get((EntityLivingBase)hit.entityHit);
          if (propz.isLogia() && getAttribute().isProjectilePhysical() && !props.hasBusoHakiActive())
            return; 
          ...
          if (this.attr.getProjectileExplosionPower() > 0) {
            AbilityExplosion explosion = WyHelper.newExplosion((Entity)getThrower(), this.posX, this.posY, this.posZ, this.attr.getProjectileExplosionPower());
          ...
          if (this.attr.getProjectileDamage() > 0.0F)
            hit.entityHit.attackEntityFrom(DamageSource.causeThrownDamage((Entity)this, (Entity)getThrower()), this.attr.getProjectileDamage() * props.getDamageMultiplier()); 
          tasksImapct(hit);
          setDead();
        } else {
          if (this.attr.getProjectileExplosionPower() > 0) {
