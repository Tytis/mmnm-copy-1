...
public class YukiAbilities {
  ...
  public static class TabiraYuki extends Ability {
    ...
    public void startPassive(EntityPlayer player) {
      if (player.inventory.getCurrentItem() == null) {
        player.inventory.setInventorySlotContents(player.inventory.currentItem, new ItemStack((Item)ListMisc.TabiraYuki));
      } else {
    ...
    public void endPassive(EntityPlayer player) {
      player.inventory.clearInventory((Item)ListMisc.TabiraYuki, -1);
    }
  }
    ...
    public void use(final EntityPlayer player) {
      if (!this.isOnCooldown) {
        ...
        for (EntityLivingBase e : WyHelper.getEntitiesNear((Entity)player, 25.0D)) {
          e.attackEntityFrom(DamageSource.causePlayerDamage(player), 35.0F);
          e.addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 200, 2));
        } 
        WyNetworkHelper.sendToAllAround((IMessage)new PacketParticles("fubuki", player.posX, player.posY, player.posZ), player.dimension, player.posX, player.posY, player.posZ, 128.0D);
        super.use(player);
      } 
    }
  }
      ...
      if (!this.isOnCooldown) {
        if (MainConfig.enableGriefing) {
          ...
          if (player.isSneaking()) {
            WyHelper.createEmptySphere(world, (int)player.posX, (int)player.posY, (int)player.posZ, 4, Blocks.snow, new String[] { "air", "foliage", "liquids" });
            WyHelper.createEmptySphere(world, (int)player.posX, (int)player.posY, (int)player.posZ, 6, Blocks.snow, new String[] { "air", "foliage", "liquids" });
            WyHelper.createEmptySphere(world, (int)player.posX, (int)player.posY, (int)player.posZ, 8, Blocks.snow, new String[] { "air", "foliage", "liquids" });
          } else if (mop != null && world.getBlock(mop.blockX, mop.blockY, mop.blockZ) != Blocks.air) {
            WyHelper.createEmptySphere(world, mop.blockX, mop.blockY, mop.blockZ, 4, Blocks.snow, new String[] { "air", "foliage", "liquids" });
            WyHelper.createEmptySphere(world, mop.blockX, mop.blockY, mop.blockZ, 6, Blocks.snow, new String[] { "air", "foliage", "liquids" });
            WyHelper.createEmptySphere(world, mop.blockX, mop.blockY, mop.blockZ, 8, Blocks.snow, new String[] { "air", "foliage", "liquids" });
          } 
        } 
        super.use(player);
      } 
  ...
  public static class YukiRabi extends Ability {
    ...
    public void use(EntityPlayer player) {
      this.projectile = (AbilityProjectile)new YukiProjectiles.YukiRabi(player.worldObj, (EntityLivingBase)player, this.attr);
      super.use(player);
    }
  }
