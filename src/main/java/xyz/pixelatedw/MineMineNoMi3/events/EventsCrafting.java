  ...
  public void onAnvilUpdate(AnvilUpdateEvent event) {
    if (ItemsHelper.isBow(event.left)) {
      ...
      if (event.right.getItem() == ListMisc.Kairoseki && event.right.stackSize >= 10) {
        ...
        if (event.output.getTagCompound().getDouble("multiplier_black_metal") >= 0.3D * level)
          return; 
        event.output.getTagCompound().setDouble("multiplier_black_metal", 0.3D * level);
        ...
        if (event.left.getTagCompound() == null)
          event.left.setTagCompound(new NBTTagCompound()); 
        event.output.setTagCompound((NBTTagCompound)event.left.getTagCompound().copy());
        if (event.output.getTagCompound().getDouble("multiplier_eisen") >= 0.05D * level)
          return; 
        event.output.getTagCompound().setDouble("multiplier_eisen", 0.05D * level);
