...
public class ParticleEffectChloroBallCloud extends ParticleEffect {
  public void spawn(EntityPlayer player, double posX, double posY, double posZ) {
    for (int i = 0; i < 128; i++) {
      ...
      double motionX = WyMathHelper.randomDouble() / 8.0D;
      double motionZ = WyMathHelper.randomDouble() / 8.0D;
      MainMod.proxy.spawnCustomParticles((Entity)player, (new EntityParticleFX(player.worldObj, ID.PARTICLE_ICON_DOKU, posX + offsetX, posY + offsetY, posZ + offsetZ, motionX, 0.05D, motionZ))
          
          .setParticleScale(1.2F).setParticleGravity(0.0F).setParticleAge(5 + player.worldObj.rand.nextInt(2)));
    } 
  }
}
