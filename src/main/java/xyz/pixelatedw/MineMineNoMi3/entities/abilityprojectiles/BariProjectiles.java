  ...
  public static class BarrierbilityStairs extends AbilityProjectile {
    ...
    public void onUpdate() {
      if (!this.worldObj.isRemote) {
        if (getThrower() == null)
          return; 
        AbilityProperties abilityProps = AbilityProperties.get((EntityPlayer)getThrower());
        ...
        if (ability != null) {
          ...
          ((BariAbilities.BarrierbilityStairs)ability).fillBlocksList(WyHelper.createFilledCube(this.worldObj, this.posX, this.posY - 2.0D, this.posZ, new int[] { 1, 1, 1 }, ListMisc.Barrier, new String[] { "air", "nogrief" }));
        } 
      } 
      super.onUpdate();
    }
